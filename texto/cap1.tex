...
%Em 2017, fora proposta a \ac{DenseNet}\cite{liu-2017}, uma arquitetura robusta de \ac{CNN} que tem em sua arquitetura blocos densos de camadas de convolução, onde cada camada de convolução $C_n$ recebe como entrada as saídas das $C_{n - 1}$ camadas anteriores. Esses blocos densos permitem que cada camada processe múltiplos níveis de abstração, tornando assim, mais robustos os resultados da classificação \cite{liu-2017}.

%%% p2 Escrever sobre os resultados obtidos pelas CNNs em Localização de Objetos
Nas tarefas de localização de objetos as \ac{CNN}s também têm se destacado com bons resultados. \citeonline{redmon-2015} propuseram o \ac{YOLO} e obtiveram um bom resultado fazendo localização e classificação de objetos em tempo real, conseguindo processar 45 \ac{FPS} com uma acurácia de 63,4\%. \citeonline{sren-2017} propuseram o \ac{R-CNN} e conseguiram obter resultados significativamente melhores para as tarefas de localização e classificação chegando a uma acurácia de até 78,8\%. Numa implementação alternativa com o enfoque no processamento em tempo real, a acurácia cai um pouco, chegando a 73,2\% porém, ele processa apenas 7 \ac{FPS}.

\citeonline{wei-2015} propuseram o \ac{SSD}, uma abordagem que além de obter uma acurácia elevada (74,3\%), supera a velocidade alcançada no \ac{YOLO} atingindo 59 \ac{FPS}. Esses resultados são especialmente relevantes, pois, além de serem competitivos com o estado da arte, trabalham com imagens menores - enquanto os quadros processados pelo \ac{YOLO} têm dimensões $448 \times 448$ e os processados pelo \ac{R-CNN} têm $1000\times 600$, o \ac{SSD} processa quadros de dimensões $300 \times 300$ - consumindo assim menos memória sem comprometer a acurácia do método.
...
\begin{itemize}
	\item Adaptar a \ac{CNN} \ac{DenseNet} \cite{liu-2017} para o problema de classificação de objetos em imagens;
	\item Adaptar a rede para fazer a localização e classificação de forma similar à \ac{SSD};
